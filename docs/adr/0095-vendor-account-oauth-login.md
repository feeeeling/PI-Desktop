...
6. **Model discovery and connection tests go through the account.** For an
   ...
   account by resolving auth. The resolved account token also calls
   `GET /models`; when that succeeds, the chat-model list replaces the pinned
   catalog so a newly released model does not wait for a client update.
   Radius already refreshes its own catalog and is not probed again. Login stores a non-secret account label in the row
