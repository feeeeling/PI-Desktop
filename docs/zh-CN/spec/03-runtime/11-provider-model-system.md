...
连接测试通过解析认证来证明账户。模型发现用同一枚账户令牌请求
`GET /models`。对 ChatGPT Plus/Pro（`openai-codex`）这类静态 OAuth 厂商，
已认证目录（`models.getAvailable`，它已应用厂商自己的 `filterModels`）
只是失败时的回退。用已解析的账户令牌请求 `GET /models` 成功时，
返回的对话模型就是该账户可选的模型，包括 pin 里还没有的 id；图像、视频、
语音和嵌入模型会被丢掉。GitHub Copilot 这类多种线路的厂商，只有当新 id
的家族（`claude`、`gpt`、`gemini` 等）在 pin 里已经对应唯一线路时才会加入，
不认识的家族不会猜线路。Radius 自己会刷新目录，不再探测一次。请求失败时
仍回退到 `getAvailable`。因此厂商列表里已经有的新模型不需要更新客户端。
models.dev 在 ID 可用后仍
